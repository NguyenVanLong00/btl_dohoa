...
window.onload = function init()
{
    ...
    canvas.addEventListener("wheel", (e) => {
        radius += (e.deltaY == 90 ? zRate : (radius >= zRate ? -zRate : 0));
...
function rotateX()
{
    for (let i = 0; i < rubic_vertices.length; i++)
        rubic_vertices[i] = mult(mat3(1,                  0,                   0,
                                      0, Math.cos(theta[0]), -Math.sin(theta[0]),
                                      0, Math.sin(theta[0]),  Math.cos(theta[0]) ), rubic_vertices[i]);

    for (let i = 0; i < grid_vertices.length; i++)
        grid_vertices[i] = mult(mat3(1,                  0,                0,
                                     0, Math.cos(theta[0]), -Math.sin(theta[0]),
                                     0, Math.sin(theta[0]),  Math.cos(theta[0]) ), grid_vertices[i]);
}
function rotateY()
{
    for (let i = 0; i < rubic_vertices.length; i++)
        rubic_vertices[i] = mult(mat3( Math.cos(theta[1]), 0, Math.sin(theta[1]),
                                                        0, 1,                  0,
                                      -Math.sin(theta[1]), 0, Math.cos(theta[1]) ), rubic_vertices[i]);

    for (let i = 0; i < grid_vertices.length; i++)
        grid_vertices[i] = mult(mat3( Math.cos(theta[1]), 0, Math.sin(theta[1]),
                                                       0, 1,                  0,
                                     -Math.sin(theta[1]), 0, Math.cos(theta[1]) ), grid_vertices[i]);
}

function rotateZ()
{
    for (let i = 0; i < rubic_vertices.length; i++)
        rubic_vertices[i] = mult(mat3(Math.cos(theta[2]), -Math.sin(theta[2]), 0,
                                      Math.sin(theta[2]),  Math.cos(theta[2]), 0,
                                                       0,                   0, 1 ), rubic_vertices[i]);

    for (let i = 0; i < grid_vertices.length; i++)
        grid_vertices[i] = mult(mat3(Math.cos(theta[2]), -Math.sin(theta[2]), 0,
                                     Math.sin(theta[2]),  Math.cos(theta[2]), 0,
                                                      0,                   0, 1 ), grid_vertices[i]);
}
